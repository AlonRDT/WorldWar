...
public abstract class CardPile : MonoBehaviour
{
    ...
    public abstract void AcceptNewCardToPile(Button_Card card);
    public abstract void RemoveCardFromPile(Button_Card card);
    public abstract void ReturnCardToPlace(Button_Card card);
    public void AddNewCard(Button_Card newCard, int index)
    {
        newCard.HoldingPileType = PileType;
        m_CardPlaceholders[index].PlaceCard(newCard);
        ReturnCardToPlace(newCard);
    }
    ...
    private void placeCardPlaceholder(int placeholderIndex, int rowAmount, int columnAmount)
    {
        ...
        float placeholderLocationYScreenFraction = startY + halfPlaceholderHeight + halfPlaceholderHeight * 2 * ((rowAmount - 1) - placeholderIndex / columnAmount);
